...
package body Workers is
   ...
   begin
      ...
      do
         ...
         Expression := Compile (Pattern & "|.*dbtbot|.*botdbt|.*[ws5][i1]nn[e3]r|.*_dbt|.*buydbt|.*pumper|.*pump[0-9][0-9]|.*[0-9][0-9]pump|.*[0-9]pump[0-9]|.*fuck[3e]d.*|.*[5s]h[1i]tty.*|.*Randy420.*|.*420Bot|.*buy420", False, Case_Sensitive);
         Start_Time := Clock;
      end Start;
